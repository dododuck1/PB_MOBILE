...
void CBow::Spawn(void)
{
    ...
    // ตั้งค่าเริ่มต้นกระสุน
    m_iClip = 10; // เริ่มต้นด้วยกระสุน 3 นัด
    m_iDefaultAmmo = 10;
...
void CBow::PrimaryAttack(void)
{
    ...
    {
        // โหมดธนู
        if (m_iClip <= 0) // ถ้าไม่มีกระสุน
        {
            SendWeaponAnim(BOW_DRAW_EMPTY, UseDecrement() != FALSE);
            return;
        }
        
        if (m_iClip == 1) // ยิงนัดสุดท้าย
        {
...
{
    if (m_pPlayer->m_rgAmmo[m_iSecondaryAmmoType] <= 81 || shootGrenade)
        return;
...
void CBow::ammoArrow(void)
{
    ...
    CBowArrow *pEnt = static_cast<CBowArrow *>(CBaseEntity::Create("bow_arrow", 
        m_pPlayer->GetGunPosition(), 
        Stock_Get_Velocity_Angle(gpGlobals->v_forward * 2000, vecArrowAngle), 
        ENT(m_pPlayer->pev)));
