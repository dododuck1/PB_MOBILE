...
void Cmini_axe::Spawn(void)
{
    ...
    m_iDefaultAmmo = miniaxe_ammo;  // Set default ammo to 4
    m_iClip = miniaxe_ammo;         // Initialize clip with 4 axes
...
BOOL Cmini_axe::Deploy(void)
{
    ...
    // ไม่ต้อง reset clip ที่นี่
    // m_iClip = miniaxe_ammo;  // <- ลบบรรทัดนี้ออก
...
{
    // Save current clip count before holstering
...
{
    if (m_iClip <= 0)  // Check if we have axes left
        return;
...
{
    if (m_iClip <= 0)  // Double check clip
        return;

    m_iShotsFired++;
    m_iClip--;  // Decrease clip instead of increasing
    ...
    // Only destroy item if we're out of axes
    if (m_iClip <= 0)
    {
        DestroyItem();
